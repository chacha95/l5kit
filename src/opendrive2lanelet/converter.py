...
class OpenDriveConverter:
    ...
    def _create_outer_lane_border(lane_borders, lane, coeff_factor) -> Border:
        ...
        InnerBorder is already saved in lane_borders, as it is the outer border of the inner neighbour of the lane.

        Args:
          lane_borders: Previous calculated lane borders of more inner lanes.
          lane: Lane for which outer border shall be created. This is specified in parameter ds of curve length.
          coeff_factor: factor of -1 or 1, dependent on which side of the reference path the lane is (right side is -1).
